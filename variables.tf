...
variable "cluster_endpoint_public_access_cidrs" {
  ...
  type        = list(string)
  default     = ["46.137.197.68/32"] //CIDR of cloudshell from where you need access, in future to be rectify

  // privtae IP of the jumphost, can be the elastic IP for static connection.
}
...
variable "capacity_type" {
  description = "A list of Capacity type we reuired for jumphost"
  type        = list(string)
  default     = []
}
...
variable "eks_jumphost_instance_type" {
  type        = string
  description = "EKS jumphost instance type"
  default     = "t3.micro"
}
variable "jumphost_desired_capacity" {
  type    = number
  default = 1
}

variable "jumphost_max_size" {
  type    = number
  default = 1
}

variable "jumphost_min_size" {
  type    = number
  default = 1
}
